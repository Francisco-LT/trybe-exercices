...
function retriveAnimalsPerLocationWithName(locations, sorted, sex) {
  ...
  locations.forEach((location) => {
    const filteredAnimals = retriveFilteredAnimalsBylocation(location).map((animal) => {
      ...
      // fazemos primeiro o map nos nomes, que serão as chaves do meu objeot, VER TESTE
      const residents = animal.residents
      // em seguida usamos em residents, para recuperar os values dos objetos
      // residents traz um objeto inteiro, mas só queremos a string de nome que stá lá.
      // queremos alterar um array de objetos para um array de strings
      .filter((resident) => {
